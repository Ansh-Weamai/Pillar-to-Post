...
export default function RoomWalkthroughReport({
  ...
  return (
    <div>
      ...
      <p className="mb-4 whitespace-pre-line text-[14px] text-ink-soft">{formatText(result.overall_summary)}</p>
      ...
      <div className="flex flex-col gap-6">
        {result.detected_elements.map((element, i) => (
          <div key={i} className="border-b border-line pb-6 last:border-b-0">
            ...
            <p className="mt-2 whitespace-pre-line text-[14px] text-ink">{formatText(element.condition_observed)}</p>
            ...
            {element.defect_signatures.length > 0 ? (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {element.defect_signatures.map((d, di) => (
                  ...
                  >
                    {d.signature} · {SEVERITY_LABEL[d.severity]} · {Math.round(d.confidence * 100)}%
                  </span>
                ))}
              </div>
            ) : (
            ...
            {element.recommended_check ? (
              <p className="mt-2 flex items-start gap-1.5 whitespace-pre-line text-[13px] text-ink-soft">
                <Info size={14} className="mt-0.5 shrink-0" /> {formatText(element.recommended_check)}
              </p>
            ) : null}
          </div>
        ))}
